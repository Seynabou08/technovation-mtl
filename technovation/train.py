...
def fit(
    ...
    for e in range(initial_epoch, epochs):
        ...
        train_log = train(model, device, criterion, optimizer, dataloaders["train"], accumulation_steps,
                          train_pbar_desc)
        ...
        if metric_improved and model_folder is not None:
            ...
            if remove_previous_ckpt:
                ...
                if isinstance(model_folder,str):
                    for fn in Path(model_folder).glob(pattern):
                        fn.unlink()
                else:
